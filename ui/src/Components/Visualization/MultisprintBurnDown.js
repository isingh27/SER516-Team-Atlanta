...
function MultisprintBurnDown({ sprintOptions }) {
  ...
  const convertDataForChart = (data) => {
    ...
    const maxDataPoints = Math.max(
      ...Object.values(sprintData).map((sprint) => sprint.length)
    );
    ...
    for (let day = 1; day <= maxDataPoints; day++) {
      ...
      const optimalPoints =
        maxStoryPoints - (day - 1) * (maxStoryPoints / daysLeft);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      for (const sprint of sprintOptions) {
        try {
          const response = await taigaService.taigaProjectBurnDownChart(
          ...
          if (response.data.status === "success") {
            ...
            const bdTempData = response.data.burndown_chart_data.days
              .map((data) => {
                const currentDate = new Date();
                const dataDate = new Date(data.day);

                if (dataDate < currentDate) {
                  return [data.id, Math.floor(data.open_points), sprint.title];
                }
