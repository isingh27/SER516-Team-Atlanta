        ...
        for task in tasks:
            ...
            for history_entry in history:
                if 'status' in history_entry['values_diff']:
                    ...
                    if (date_str, task_id) not in unique_tasks_for_day:
                        unique_tasks_for_day.add((date_str, task_id))

                        if date_str not in task_history:
                            task_history[date_str] = {
                                'new': 0, 'in progress': 0,
                                'ready for test': 0, 'blocked': 0, 'done': 0}

                        if status:
                            task_history[date_str][status] += 1
